create or replace procedure sp_get_inv_energy_totalex(
...
begin
   ...
   for curt in (select nvl(byname,isno) byname,isno from tb_inverter where stationid =i_stationid) loop
      if instr(i_isnolist,curt.isno,1,1)>0  or (SUBSTR (i_isnolist,1,7)='23Q0001' and SUBSTR (curt.isno,1,7)='23Q0001' )  then
         ...
         select count(1) into v_r from tb_invdaily where isno =curt.isno and stationid=i_stationid;
         if v_r >0 then
       ...
       insert into tmp_info(sv1,sv2,sv3,nv1) values( curt.isno,curt.byname,to_char(v_curdate,'yyyy'),0);-- from (select distinct to_char(recvdate,'yyyy') year from tb_invdaily where stationid = i_stationid) a,(select to_char(v_curdate,'yyyy') year,v_t e_val from dual) b where a.year = b.year(+);
         end if;
         if v_t > 0 then
   ...
   for curx in (select sv3 from tmp_info) loop
     for cury in (select sv1,sv2 from tmp_info) loop 
       select count(1) into v_m from tmp_info  where sv1 = cury.sv1 and sv3 = curx.sv3;
       if v_m<=0 then 
          insert into tmp_info(sv1,sv2,sv3,nv1,nv2) values(cury.sv1,cury.sv2,curx.sv3,0,1);
       end if;
     end loop;
   end loop;
   commit;   
   open cur_result for select sv1 isno,sv2 byname,sv3 year,round(nv1/v_r,3) data1/*e_total*/,v_u e_total_unit,/*nv2*/1 res_num from tmp_info order by sv1,sv3;
end sp_get_inv_energy_totalex;
/
