create or replace procedure sp_web_query_userlist_page(
...
begin
  ...
  open cur_result for
      select a.userid,a.account,a.createdt,a.firstname,a.secondname,nvl(b.context,'nodefine') country,nvl(c.province,'nodefine') province,a.city,a.tel,a.mobile,a.company from (select * from
            (
            select x.*,rownum rn from(select userid,account,to_char(createdt,'yyyy-mm-dd') createdt,firstname,secondname,country,country*1000+province province,city,tel,mobile,COMPANY from tb_user
            where (i_country is null or country = to_number(i_country))
            and (i_province is null or province= to_number(i_province))
            and (i_city is null or city like '%'||i_city||'%')
            and (v_account is null or account like '%'||v_account||'%')
            ...
            )c  where a.country=b.country(+) and a.province = c.provinceid(+);
end sp_web_query_userlist_page;
/
