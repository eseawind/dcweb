create or replace procedure SP_GET_INV_TEMP_DAYex(
...
/*================================================================
功能说明:查询指定电站，指定逆变器列表，指定日期的10分钟温度数据
------------------------------------------------------------------
相关WEB页面及文件
...
begin
   ...
   for curt in (select nvl(byname,isno)byname ,isno from tb_inverter where stationid = i_stationid) loop
      if instr(i_isnolist,curt.isno,1,1)>0  or (SUBSTR (i_isnolist,1,7)='23Q0001' and SUBSTR (curt.isno,1,7)='23Q0001' )  then
         v_sql:='insert into tmp_info(idd,sv1,sv2,sv3,sv4,nv1)
                  select a.idd,:isno_ ,:byname_,:date_,fn_gettimebyfen10(a.idd),nvl(b.tempval,0) from tb_serial a ,
   ...
   commit;
   open curmain for select sv1 isno,sv2 byname,sv3 recvdt,sv4 rtime,idd fen10,nv1 data1/*tempval*/,'°C' unit,1 res_num from tmp_info order by isno,recvdt,fen10;
end SP_GET_INV_TEMP_DAYex;
/
