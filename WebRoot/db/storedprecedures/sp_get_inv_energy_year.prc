create or replace procedure sp_get_inv_energy_year(
i_stationid number,
i_isnolist varchar2,
i_year varchar2,--输入查询日期
o_startdt out varchar2,
o_enddt out varchar2,
cur_result out sys_refcursor--输入结果集
) is
/*================================================================
功能说明:查询指定电站,指定逆变器列表，指定年份各月的发电量情况
------------------------------------------------------------------
相关WEB页面及文件
    菜单路径:Graphs/Energy&Power/Year
...
begin
  ...
  if v_year > 2000  and v_year < 2050 then
     ...
     open cur_result for select sv2 isno,sv3 recvdate,idd,round(nv1/v_rate,3) data1/*e_total*/,v_unit e_total_unit,1 res_num from tmp_info order by sv2,sv3;
  end if;
end sp_get_inv_energy_year;
/
