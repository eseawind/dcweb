create or replace procedure sp_web_query_event_pageex(
...
begin
  ...
  if i_mode is null then
      ...
      tb_station b,(select val1,val2,context from tb_dict_ssiis where subtag = 'pmuerrcode' and language = 'zh_CN') c,(select did,stationid,ssno,occdt,val1,val2,l_tag,devt from tb_event_all where occdt between to_date(v_startdt||' 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date(v_enddt||' 23:59:59','yyyy-mm-dd hh24:mi:ss')) a
      where a.stationid=b.stationid and a.val1 = c.val1 and a.val2= c.val2 and
          ...
          (i_ssno is null or a.ssno =upper(i_ssno)) and
    ...
    o_pages:=trunc((v_rownum +v_rowperpage -1)/v_rowperpage,0);
     ...
     if i_devtype = 1 then--查找监控器--完成优化
       ...
       from (   select did,stationid,ssno,occdt,val1,val2,l_tag,devt from tb_event_all
                where occdt between to_date(v_startdt||' 00:00:00','yyyy-mm-dd hh24:mi:ss')
           ...
           select count(1) into v_rownum from (select did,stationid,ssno,occdt,val1,val2,l_tag,devt from tb_event_all where occdt between to_date(v_startdt||' 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date(v_enddt||' 23:59:59','yyyy-mm-dd hh24:mi:ss')) a,tb_inverter b,tb_station c,(select val1,val2,context from tb_dict_ssiis where subtag = 'pmuerrcode' and language = 'zh_CN') d
    ...
    if i_mode is null then
        open cur_result for
        select * from(select rownum rn,x.* from(select b.stationname,a.ssno,decode(a.val1,1,'信息',2,'警告',3,'错误','其他') msgtype,a.occdt,c.context msg_desc,b.country,b.city,a.devt  from
          (
       ...
       if i_devtype = 1 then--查找监控器
        ...
        (
             ...
             (
                  select a.stationid,a.ssno,decode(a.val1,1,'信息',2,'警告',3,'错误','其他') msgtype,to_char(a.occdt,'yyyy-mm-dd hh24:mi:ss') occdt,d.context msg_desc,e.context country,a.devt
                  from (select did,stationid,ssno,occdt,val1,val2,l_tag,devt from tb_event_all where occdt between to_date(v_startdt||' 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date(v_enddt||' 23:59:59','yyyy-mm-dd hh24:mi:ss') order by occdt desc) a,
           ...
           (
               select rownum rn,x.* from
              (
                  select b.stationname,a.ssno,decode(a.val1,1,'信息',2,'警告',3,'错误','其他') msgtype,to_char(a.occdt,'yyyy-mm-dd hh24:mi:ss') occdt,c.context msg_desc,nvl(e.context,'未知') country,b.city,a.devt
                  from (select did,stationid,ssno,occdt,val1,val2,l_tag,devt from tb_event_all where occdt between to_date(v_startdt||' 00:00:00','yyyy-mm-dd hh24:mi:ss') and to_date(v_enddt||' 23:59:59','yyyy-mm-dd hh24:mi:ss') order by occdt desc) a,tb_station b,(select val1,val2,context from tb_dict_ssiis where subtag = 'pmuerrcode' and language = 'zh_CN') c,tb_inverter d,(select val1,context from tb_dict_ssiis where subtag = 'webcountrylist' and language = 'zh_CN') e
