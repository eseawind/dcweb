create or replace procedure sp_dcg_dataup(
...
begin
   ...
   execute immediate v_sql using
                 upper(i_psno),upper(i_isno),i_stationid,i_timezone,trunc(v_querydate),v_fen10,
    ...
    update tb_pmu set ludt = v_querydate where psno = upper(i_psno);
   commit;
end sp_dcg_dataup;
/
