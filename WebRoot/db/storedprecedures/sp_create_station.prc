create or replace procedure sp_create_station(
...
begin
   ...
   if v_rownum >0 then
      ...
      insert into tb_station(stationid,stationname,createdt,masterid,startdt,totalpower,country,province,city,street,zip,jd,wd,h,comangle,co2xs,gainxs,money,timezone,w_keyval,pmutnum,pmu1num,inv1num,sharet,iconindex,bgurl) values (seq_stationid.nextval,i_stationname,sysdate,i_userid,v_startdate,i_installcap,to_number(i_country),to_number(i_province),i_city,i_street,i_zip,v_jd,v_wd,i_height,i_insangle,i_co2rate,i_incomerate,i_currency,i_timezone,i_timezonex,0,0,0,0,nvl(i_picurl,'/res/def/icon/plant01.jpg'),'/upload/station/defultBg.jpg');
