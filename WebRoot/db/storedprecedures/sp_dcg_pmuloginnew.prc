create or replace procedure sp_dcg_pmuloginnew(
      ...
      if v_stationid >0 then--已经绑定电站的PMU
         if  true then --v_idex = i_idex  暂未判断验证码部分
           v_curdate:=fn_get_station_local_date(v_stationid);--获得该电站的所在时区的当前时间
            update tb_pmu set state = 1,llip = i_clientip,lldt=v_curdate,ludt=v_curdate,totalspace=i_totalspace where psno = upper(i_psno);
