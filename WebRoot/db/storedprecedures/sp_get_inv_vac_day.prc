create or replace procedure SP_GET_INV_VAC_DAY(
...
/*================================================================
功能说明:查询指定电站，指定逆变器列表，指定日期的10分钟交流电压值数据
------------------------------------------------------------------
相关WEB页面及文件
    菜单路径:Graphs/Vac/Day
...
begin
   ...
   for curt in (select nvl(byname,isno)byname ,isno from tb_inverter where stationid = i_stationid) loop
      if instr(i_isnolist,curt.isno,1,1)>0 or (SUBSTR (i_isnolist,1,7)='23Q0001' and SUBSTR (curt.isno,1,7)='23Q0001' )  then
         v_sql:='insert into tmp_info(idd,sv1,sv2,sv3,nv1,nv2,nv3)
                  select a.idd,:isno_ ,:date_,fn_gettimebyfen10(a.idd),nvl(b.vac1,0),nvl(b.vac2,0),nvl(b.vac3,0) from tb_serial a left join
   ...
   commit;
   open curmain for select sv1 isno,sv2 recvdt,sv3 rtime,idd fen10,nv1 data1/*vac1*/,nv2 data2/*vac2*/,nv3 data3/*vac3*/,'V' unit,3 res_num from tmp_info order by isno,sv1,fen10;
end SP_GET_INV_VAC_DAY;
/
