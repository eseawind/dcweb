create or replace procedure sp_get_inv_energy_dayex(
...
begin
   ...
   for curt in (select nvl(byname,isno)byname ,isno from tb_inverter where stationid = i_stationid) loop
      if instr(i_isnolist,curt.isno,1,1)>0 or (SUBSTR (i_isnolist,1,7)='23Q0001' and SUBSTR (curt.isno,1,7)='23Q0001' ) then
         v_sql := 'insert into tmp_info(sv1,sv2,sv3,nv1,nv2,nv3)
         select :isno__ isno,:byname__ byname,fn_gettimebyfen10(a.idd) rtime,a.idd fen10,nvl(b.pac,0) pac,nvl(b.pdc,0) pdc from
            tb_serial a,
            (
          ...
          execute immediate v_sql using curt.isno,curt.byname,curt.isno,i_stationid;
      end if;
   end loop;
  -- select nvl(max(nv2),0) into v_maxpac from tmp_info ;
   ...
   commit;
   open cur_result for select sv1 isno,sv2 byname,sv3 rtime,nv1 fen10,round(nv2/v_rate,3) data1/*pac*/,round(nv3/v_rate,3) data2,v_unit unit,1 res_num from tmp_info order by isno,fen10;
end sp_get_inv_energy_dayex;
/
