create or replace procedure sp_dcg_pmuloginnewex(
i_dcgate varchar2,--DCGate名称
i_psno varchar2,--pmu 序列号
i_idex out varchar2,--pmu设备序列号
i_clientip varchar2,--pmu ip地址
i_totalspace number,--总空间
      ...
      if v_stationid >0 then--已经绑定电站的PMU
         if  true then --v_idex = i_idex  暂未判断验证码部分
           v_curdate:=fn_get_station_local_date(v_stationid);--获得该电站的所在时区的当前时间
            update tb_pmu set state = 1,llip = i_clientip,lldt=v_curdate,ludt=v_curdate,totalspace=i_totalspace,CURGATE=i_dcgate where psno = upper(i_psno);
