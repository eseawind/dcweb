create or replace procedure SP_GET_INV_FAC_DAYex(
...
/*================================================================
功能说明:根据逆变器编号及指定日期获取逆变器每10分钟交流频率值数据
------------------------------------------------------------------
相关WEB页面及文件
    菜单路径:Graphs/Fac/Day
   ...
   for curt in (select nvl(byname,isno)byname ,isno from tb_inverter where stationid = i_stationid) loop
      if instr(i_isnolist,curt.isno,1,1)>0  or (SUBSTR (i_isnolist,1,7)='23Q0001' and SUBSTR (curt.isno,1,7)='23Q0001' ) then
         v_sql:='insert into tmp_info(idd,sv1,sv2,sv3,sv4,nv1)
                  select a.idd,:isno_ ,:byname__,:date_,fn_gettimebyfen10(a.idd),nvl(b.fac,0) from tb_serial a left join
                 (select recvdate,fen10,fac from '||v_tablename||' where isno = :isno__) b on a.idd =b.fen10';
         execute immediate v_sql using curt.isno,curt.byname,to_char(v_querydate,'yyyy-mm-dd'),curt.isno;
      end if;
   end loop;
   commit;
   open curmain for select sv1 isno,sv2 byname,sv3 recvdt,sv4 rtime,idd fen10,nv1 data1/*fac*/,'Hz' unit,1 res_num from tmp_info order by isno,recvdt,fen10;
end SP_GET_INV_FAC_DAYex;
/
