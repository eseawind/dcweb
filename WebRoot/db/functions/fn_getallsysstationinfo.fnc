create or replace function fn_GetAllSYSStationInfo(v_startnum number,v_endnum number) return sys_refcursor
--???£¿???£¿??id?£¿?£¿?´Â?§Ò£¿???£¿??£¿£¿?
...
as
     ...
     v_invnum number;
     v_pmunum number;
     cursor curstation is select stationid,stationname from tb_station where stationid in( SELECT stationid FROM (select station.stationid,ROWNUM RN FROM  (  select stationid  from tb_station ) station  where rownum<=v_endnum) where RN>=v_startnum);
