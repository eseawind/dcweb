...
public class invDownload extends HttpServlet {

	private static Logger logger = Logger.getLogger(invDownload.class);
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) {
		...
		String fileName=getProperties("RES_DOWNLOAD_INV_TITLE",realPath+"messageResource_"+localLang+".properties");
		SimpleDateFormat formatter = new SimpleDateFormat("yyyyMMddHHmmss");
		String nowTime= formatter.format(new Date());
		fileName=fileName+"_"+nowTime;	
		try {
			List<Map> pmuList=stationDao.findInvList(stationName,model,lsno, 100000, 1);
			...
			String[] contents = {getProperties("RES_SERIALNUMBER",realPath+"messageResource_"+localLang+".properties"),
					getProperties("RES_CONF_FACTORY",realPath+"messageResource_"+localLang+".properties"),
					getProperties("RES_CONF_PINPAI",realPath+"messageResource_"+localLang+".properties"),
					getProperties("RES_CONF_MODEL",realPath+"messageResource_"+localLang+".properties"),
					getProperties("RES_ESTATUS",realPath+"messageResource_"+localLang+".properties"),
					getProperties("RES_STATIONNAME",realPath+"messageResource_"+localLang+".properties"),
			...
			if (pmuList!=null && pmuList.size()>0){
				try {          
					for (int i=0;i<pmuList.size();i++){
						...
						ds[1]=(inv.get("factoryname")==null?"":inv.get("factoryname").toString());
						ds[2]=(inv.get("penpai")==null?"":inv.get("penpai").toString());
						ds[3]=(inv.get("invtype")==null?"":inv.get("invtype").toString());
						ds[4]=(Integer.parseInt(inv.get("state").toString())==1?getProperties("RES_ONLINE",realPath+"messageResource_"+localLang+".properties"):getProperties("RES_OFFLINE",realPath+"messageResource_"+localLang+".properties"));
						ds[5]=(inv.get("stationname")==null?"":inv.get("stationname").toString());
 	           	...
 	           	try {     
 	           		if (pmuList!=null && pmuList.size()>0){
 						for (int i=0;i<pmuList.size();i++){
 							...
 							ds=ds+"\t"+(inv.get("factoryname")==null?"":inv.get("factoryname").toString());
 							ds=ds+"\t"+(inv.get("penpai")==null?"":inv.get("penpai").toString());
 							ds=ds+"\t"+(inv.get("invtype")==null?"":inv.get("invtype").toString());
