...
<script type="text/javascript">
	DCPOWER.init('<%= JSONArray.fromObject(allInvList).toString() %>',"inve",'<%= request.getAttribute("type").toString() %>','<%= request.getAttribute("sdate").toString() %>');
</script>
<div class="rightdh">
	<b class="dot_nav"></b><s:text name="RES_CURRENTLOCAL"/>: <a href="javascript:;" onclick="drawmenu.getBody('powerlist.action');return false;"><s:text name="RES_HOME"/></a> > <a href="#"><s:text name="RES_INVE"/></a>
</div>
<div class="rightcon">
	<div class="rightcon_tcon">
		<div class="container">
			<div class="title"><s:text name="RES_INVE"/> ( <s:property value="#session.defaultStationMap.stationname" /> ) </div>
			<div class="dot_con">
				<input id="t" type="hidden" value="inve" />
					...
					<td align="left" style="padding-left:10px;">
						<%  
							...
							for(int i=0;i<allInvList.size();i++){
								...
								if(("0").equals(channels.trim())){
									flag=1;
						%>
									<div class="invbtncon">
										<div class="invbtn_l"></div>
										<div class="invbtn_c">
											<div class="invbtn_ctxt"><%= allInvList.get(i).get("byName") %></div>
											<% 
												if(editInvLimit==1){
											%>
											<div class="invbtn_cclose" onclick="DCPOWER.deleteChannel2('<%= allInvList.get(i).get("isno").toString() %>')"></div>
											<% 
												}
											 %>
										</div>
		...
		<div class="p_rtb" style="float: left;">
			<div class="p_rttab" style="left: 10px;">
					<div class="tab" linked='inve.action?type=day&sdate=<s:property value="sdate"/>'>
						<s:if test="type=='day'">
					...
					<div class="tab" style="margin-left: 10px;" linked='inve.action?type=week&sdate=<s:property value="sdate"/>'>
						<s:if test="type=='week'" >
							<div class="tableft_on"></div>
		...
		<div class="e_bluecon" style="height: auto;">
			...
			<div style="width: 90%;height: 700px;margin: 0 auto;padding-top: 20px;">
				<div id="flashCont">
						 <script type="text/javascript">
							 	...
							 	showFlashElement("swf/TempDaily.swf",flashvars);
						 </script>
				</div>
			</div>
			</s:if>
			...
			<div style="width: 90%;height: 700px;margin: 0 auto;padding-top: 20px;">
				<div id="flashCont">
						 <script type="text/javascript">
							 	var flashvars = {"lang":"<%= lang %>","isno":"<s:property  value='isno'/>","baseUrl":"<%= baseUrl %>","type":"day","date":"<s:property  value='sdate'/>"}
							 	showFlashElement("swf/TempWeekly.swf",flashvars);
						 </script>
				</div>
			</div>
			</s:elseif>
			<div style="width: 90%;margin: 0 auto;">
				<table id="Searchresult" cellpadding="0" cellspacing="0" border="0"  style="border-top: 1px solid #ccc;border-left: 1px solid #ccc;border-right: 1px solid #ccc;border-bottom: 1px solid #ccc;margin-top:20px;line-height:30px;width: 100%;">
					<tr style="background: url('images/bg_tab.jpg') 0px -197px repeat-x;height: 30px;line-height: 30px;">
						<td><s:text name="RES_INVNAME" /></td><td><s:text name="RES_INVISNO" /></td><td><s:text name="RES_MAX" /></td><td><s:text name="RES_MIN" /></td><td><s:text name="RES_AVG" /></td>
					</tr>
					<%
