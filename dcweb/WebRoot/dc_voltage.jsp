...
<script type="text/javascript">
	DCPOWER.init('<%= JSONArray.fromObject(allInvList).toString() %>',"dcv",'<%= request.getAttribute("type").toString() %>','<%= request.getAttribute("sdate").toString() %>');
</script>

<div class="rightdh">
	<b class="dot_nav"></b><s:text name="RES_CURRENTLOCAL"/>: <a href="javascript:;" onclick="drawmenu.getBody('powerlist.action');return false;"><s:text name="RES_HOME"/></a> > <a href="#"><s:text name="RES_DCV"/></a>
</div>
<div class="rightcon">
	<div class="rightcon_tcon">
		<div class="container">
			<div class="title"><s:text name="RES_DCV"/> ( <s:property value="#session.defaultStationMap.stationname" /> ) </div>
			<div class="dot_con">
				<input id="t" type="hidden" value="dcv" />
				...
				<div class="_link" onclick="download.txtdownload();">
					...
					<div style="float: left;" onclick="download.cvsdownload();"><span class="spantitle"><s:text name="RES_TEXT" /></span></div>
				</div>
				<div class="_link" onclick="download.printCharts();">
					<div class="dot_print" style="margin-top: 24px;"></div>
					<div style="float: left;" onclick="download.printCharts();"><span class="spantitle"><s:text name="RES_PRINT" /></span></div>
				</div>
			</div>
		</div>
		<div class="e_bluecon">
		...
		<div class="p_rtb" style="float: left;">
			<div class="p_rttab" style="left: 10px;">
					<div class="tab" linked='dcv.action?type=day&sdate=<s:property value="sdate"/>'>
						<s:if test="type=='day'">
					...
					<div class="tab" style="margin-left: 10px;" linked='dcv.action?type=week&sdate=<s:property value="sdate"/>'>
						<s:if test="type=='week'" >
							<div class="tableft_on"></div>
				...
				<div style="width: 90%;height: 700px;margin: 0 auto;padding-top: 20px;">
					<div id="flashCont">
							 <script type="text/javascript">
								 	...
								 	showFlashElement("swf/DCVDaily.swf",flashvars);
							 </script>
					</div>
				</div>
			</s:if>
			<s:elseif test="type=='week'">
				<div style="width: 90%;height: 700px;margin: 0 auto;padding-top: 20px;">
					<div id="flashCont">
							 <script type="text/javascript">
								 	var flashvars = {"lang":"<%= lang %>","isno":"<s:property  value='isno'/>","baseUrl":"<%= baseUrl %>","type":"day","date":"<s:property  value='sdate'/>"}
								 	showFlashElement("swf/DCVWeekly.swf",flashvars);
							 </script>
					</div>
				</div>
			</s:elseif>
