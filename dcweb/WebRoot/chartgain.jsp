...
function viewData(Str){
		...
		var nodes = XMLJS.getXMLChildNodes(root);
		    var tbls="<div id='bbb' style='height:400px; width:520px;  overflow-y:auto; overflow-x:no'>";
			  ...
			  tbls=tbls+"                <td width='50%' align='center' class='black'>"+RES.RES_CHART_DOWN_YIELD+"("+unit+")</td>";
			  tbls=tbls+"              </tr>";
			  tbls=tbls+"          </table></td>";
			  tbls=tbls+"        </tr>";
			  tbls=tbls+"        <tr>";
		...
		if (tableid==1101){
		 obj=RES.RES_CHART_DOWN_OBJ_1101;
		 til=til+RES.RES_CHART_DOWN_YIELD+"("+unit+")";
		}
		else if (tableid==1102){
		 obj=RES.RES_CHART_DOWN_OBJ_1102;
		 til=til+RES.RES_CHART_DOWN_YIELD+"("+unit+")";
		}
		else if (tableid==1103){
		 obj=RES.RES_CHART_DOWN_OBJ_1103;
		 til=til+RES.RES_CHART_DOWN_YIELD+"("+unit+")";
		}
		else if (tableid==1104){
		 obj=RES.RES_CHART_DOWN_OBJ_1104;
		 til=til+RES.RES_CHART_DOWN_YIELD+"("+unit+")";
		}
    ...
    String baseUrl = request.getScheme()+"://"+request.getServerName()+":"+request.getServerPort()+request.getContextPath()+"/" ;
     %>
            ...
            var chartName="<s:text name="RES_CHART2"/>";
       		chartName=sp1 + chartName + sp1 + "<s:text name="RES_INCOME"/>" + sp1;
        ...
        showFlashElement("swf/Gain.swf", flashvars);
    </script>
           </td>
          </tr>
        </table></td>
