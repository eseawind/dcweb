...
/**
 * @see 获取电站收益数据信息, 日,月,年,总计
 */
@SuppressWarnings({"serial","unchecked"})
public class StationYieldInfo extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if(StringHelper.isCanQuery(info)){
				switch (tabid) {
					case 1101:
						dataList = stationDao.getChartDataGain7Day(Integer.parseInt(sid), date);
						break;
					case 1102:
						dataList = stationDao.getChartDataGain12Month(Integer.parseInt(sid), date);
						break;
					case 1103:
						dataList = stationDao.getChartDataGainOneYear(Integer.parseInt(sid), date);
						break;
					case 1104:
						dataList = stationDao.getChartDataGainTotal(Integer.parseInt(sid));
						break;
					default:
						dataList=null;
				}
			...
			out.println("Request format: http://solarcloud.zeversolar.com/stationYieldInfo?key=YWRtaW5AemV2ZXJzb2xhci5jb21fMTA2Ng==&period=by7days&date=2014-04-11");
		}
		out.flush();
		out.close();
	}
